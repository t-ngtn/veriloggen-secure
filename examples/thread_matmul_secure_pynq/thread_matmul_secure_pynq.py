...
def mkMatmul():
    ...
    def matmul():
        while True:
            ...
            # maxi.init()  # if matrix_size > 13, init() does not work
    ...
    def comp(matrix_size, a_offset, b_offset, c_offset):
        ...
        for i in range(matrix_size):
            maxi.dma_read_secure(ram_a, 0, a_addr, matrix_size)

            b_addr = b_offset
            for j in range(matrix_size):
                maxi.dma_read_secure(ram_b, 0, b_addr, matrix_size)
            ...
            maxi.dma_write_secure(ram_c, 0, c_addr, matrix_size)
            a_addr += matrix_size * (datawidth // 8)
